...
class FabricVenusConfigTest {
    ...
    @Test
    fun `V33 save uses locale independent decimal values`() {
        val previous = Locale.getDefault()
        val dataFolder = createTempDirectory("venus-fabric-config").toFile()
        try {
            Locale.setDefault(Locale.GERMANY)
            FabricVenusConfig(dataFolder, logger()).save(BackendConfig(backgroundOpacity = 0.4f))

            assertTrue(dataFolder.resolve("config.yml").readText().contains("background_opacity: 0.40"))
            assertEquals(0.4f, FabricVenusConfig(dataFolder, logger()).load().backgroundOpacity)
        } finally {
